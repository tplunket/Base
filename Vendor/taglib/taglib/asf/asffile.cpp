...
void ASF::File::read(bool /*readProperties*/, Properties::ReadStyle /*propertiesStyle*/)
{
  ...
  for(int i = 0; i < numObjects; i++) {
    ByteVector otherGuid = readBlock(16);
    if(otherGuid.size() != 16) {
      setValid(false);
      break;
    }
    ...
    BaseObject *obj;
    if(otherGuid == filePropertiesGuid) {
      obj = new FilePropertiesObject();
    }
    else if(otherGuid == streamPropertiesGuid) {
      obj = new StreamPropertiesObject();
    }
    else if(otherGuid == contentDescriptionGuid) {
      obj = new ContentDescriptionObject();
    }
    else if(otherGuid == extendedContentDescriptionGuid) {
      obj = new ExtendedContentDescriptionObject();
    }
    else if(otherGuid == headerExtensionGuid) {
      obj = new HeaderExtensionObject();
    }
    else {
      if(otherGuid == contentEncryptionGuid ||
         otherGuid == extendedContentEncryptionGuid ||
         otherGuid == advancedContentEncryptionGuid) {
        d->properties->setEncrypted(true);
      }
      obj = new UnknownObject(otherGuid);
    }
    obj->parse(this, size);
