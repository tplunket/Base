...
bool Ogg::File::nextPage()
{
  ...
  for(uint i = 0; i < d->currentPage->packetCount(); i++) {
    uint otherCurrentPacket = d->currentPage->firstPacketIndex() + i;
    if(d->packetToPageMap.size() <= otherCurrentPacket)
      d->packetToPageMap.push_back(List<int>());
    d->packetToPageMap[otherCurrentPacket].append(d->pages.size() - 1);
  }
