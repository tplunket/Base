...
{
public:
  ValueReader(T &newValue) : value(newValue)
  {
  }
...
{
public:
  NumberReader(T &value, bool newBigEndian) :
    ValueReader<T>(value), bigEndian(newBigEndian)
  {
  }
    ...
    for(ushort j = 0; j < sampleCount; ++ j) {
      if(sampleHeaderSize > 4U) {
        ...
        if(sampleHeaderSize > 18U) {
          seek(pos + 18);
          const uint otherLen = std::min(sampleHeaderSize - 18U, 22UL);
          if(sampleNameIndex >= lines.size())
            writeString(String::null, otherLen);
          else
            writeString(lines[sampleNameIndex ++], otherLen);
        }
      }
...
void XM::File::read(bool)
{
  ...
  for(ushort i = 0; i < patternCount; ++ i) {
    ...
    uint otherCount = pattern.read(*this, patternHeaderLength - 4U);
    READ_ASSERT(otherCount == std::min(patternHeaderLength - 4U, (ulong)pattern.size()));

    seek(patternHeaderLength - (4 + otherCount) + dataSize, Current);
  }
  ...
  for(ushort i = 0; i < instrumentCount; ++ i) {
    ...
    uint otherCount = 4 + instrument.read(*this, instrumentHeaderSize - 4U);
    READ_ASSERT(otherCount == std::min(instrumentHeaderSize, (ulong)instrument.size() + 4));
    ...
    if(sampleCount > 0) {
      ...
      READ_ASSERT(instrumentHeaderSize >= otherCount + 4 && readU32L(sampleHeaderSize));
      // skip unhandeled header proportion:
      seek(instrumentHeaderSize - otherCount - 4, Current);
      ...
      for(ushort j = 0; j < sampleCount; ++ j) {
        ...
        uint yetAnotherCount = sample.read(*this, sampleHeaderSize);
        READ_ASSERT(yetAnotherCount == std::min(sampleHeaderSize, (ulong)sample.size()));
        // skip unhandeled header proportion:
        seek(sampleHeaderSize - yetAnotherCount, Current);
      ...
      offset = instrumentHeaderSize - otherCount;
    }
